...
class C2cPlatformService extends BaseService
{
    ...
    public function matchOrders(string $currency): void
    {
        ...
        //买入，当行情价小于等于限价时则交易
        foreach ($buyOrderDetails as $buyOrderNo => $buyOrder){
            $uid = $buyOrder['user_id'];
            $id = $buyOrder['uuid'];
            //交易对
            $symbol = strtolower($buyOrder['currency']);
            //交易对象
            $current = explode('/', $currency)[0];
            //付款币种
            $pay_currency = explode('/', $currency)[1];
...
//            $fee = $buyOrder['fee'];
            $fee = 0;
            //当前行情价
            $market_price = 91222.03;
            //情价小于等于限价时则交易
            if ($market_price <= $unit_price){
                //交易的数量 付了多少钱 usdt
                $pay_amount = $buyOrder['total_price'];
                //计算能够买到的量
                $can_add = sprintf("%.8f",($pay_amount / $market_price));
                //手续费
                $fee = $can_add * $fee / 100;
                //实际到账号的金额
                $real_amount = $can_add - $fee;
                //增加用户此币种的余额
                try {
                    //扣除用户此币种的冻结金额
                    (new FinancialService())->ChangeUserMoney($buyOrder['user_id'],$pay_currency, $pay_amount, $buyOrder['order_no'], 'reduce_freeze', 'c2c_order');
                    //增加用户此币种的余额
                    (new FinancialService())->ChangeUserMoney($buyOrder['user_id'],$current, $real_amount, $buyOrder['order_no'], 'add', 'c2c_order');
                    //更新订单状态
                    //更新订单状态
                    ...
                    // 从有序集合中删除订单
                    $orderKey = $this->buyOrdersKey;
                    Redis::zRem("$orderKey:$this->currency", $buyOrder['order_no']);
                    Redis::hDel($this->inventoryKey, $buyOrder['order_no']);
                    // 删除缓存中的订单信息
                    Redis::hDel($this->ordersInfoKey, $buyOrder['order_no']);
                } catch (\Exception $e) {
                    $this->log()->error("增加用户{$uid}此币种{$symbol}的余额失败: {$e->getMessage()}");
                    continue;
                }
        ...
        foreach ($sellOrderDetails as $sellOrderNo => $sellOrder){
            //卖出的数量
            $amount = $sellOrder['amount'];
            //行情价
            $market_price = 92222.03;
            //限价单价
            //手续费比例
//            $fee = $buyOrder['fee'];
            $fee = 0;
            ...
            if ($market_price >= $unit_price){
                //求出卖出所得的目标币种数量
                $target_amount  = sprintf("%.8f",($amount * $market_price));
                //求出手续费
                $fee = $target_amount *  $fee / 100;
                //求出实际到账USDT量
                $can_add = $target_amount - $fee;
                //增加用户此币种的余额

                //交易对象
                $current = explode('/', $currency)[1];
                //付款币种
                $pay_currency = explode('/', $currency)[0];
                try {
                    //扣除用户此币种的冻结金额
                    (new FinancialService())->ChangeUserMoney($sellOrder['user_id'],$pay_currency, $amount, $sellOrder['order_no'], 'reduce_freeze', 'c2c_order');
                    //增加用户此币种的余额
                    (new FinancialService())->ChangeUserMoney($sellOrder['user_id'],$current, $can_add, $sellOrder['order_no'], 'add', 'c2c_order');
                    //更新订单状态
                    //更新订单状态
                    ...
                    // 从有序集合中删除订单
                    $orderKey = $this->sellOrdersKey;
                    Redis::zRem("$orderKey:$this->currency", $sellOrder['order_no']);
                    Redis::hDel($this->inventoryKey, $sellOrder['order_no']);
                    // 删除缓存中的订单信息
                    Redis::hDel($this->ordersInfoKey, $sellOrder['order_no']);
                } catch (\Exception $e) {
                    $this->log()->error("增加用户{$sellOrder['user_id']}此币种{$sellOrder['currency']}的余额失败: {$e->getMessage()}");
                    continue;
                }
